...
test('GUI auto-off accepts only explicit supported durations', () => {
  assert.deepEqual(GUI_AUTO_OFF_DEFAULTS, { Purge_MODE: 30, Drain_MODE: 60 });
...
test('maintenance interlock ignores non-maintenance modes and the requested mode', () => {
  assert.equal(activeMaintenanceMode({ Purge_MODE: '1', Run_MODE: '1' }, 'Drain_MODE'), 'Purge_MODE');
